...
random_points = points_in_5d_ball(50000)
random_points_check = points_in_5d_ball(100000)
...
# To avoid the program to always calculate new random data, we save the random set once it has been calculated for the first time.

Input_path_dim5 = 'Random_Input_width6_dim5.npy'
Input_path_dim5_check = 'Random_Input_width6_dim5_check.npy'
...
Output_path_dim5 = 'Random_Output1_width6_dim5.npy'
...
infinity_norms_rand2_dim5 = []
infinity_norms_rand8_dim5 = []

# We train our six different networks 10 times and save loss, maximizer and infinity norm for each epoch
for i in range(10):
    ...
    for epoch in range(epochs):
        ...
        history2 = model_depth2_random.fit(random_input, random_output, epochs=1, batch_size=batch_size, verbose=0)
        history3 = model_depth8_random.fit(random_input, random_output, epochs=1, batch_size=batch_size, verbose=0)
        ...
        sup_norm2, argmax_point2 = infinity_norm_difference(model_depth2_random, f, random_input_check)
        sup_norm3, argmax_point3 = infinity_norm_difference(model_depth8_random, f, random_input_check)
        loss_log_rand1_dim5.append([i, epoch + 1, loss1, sup_norm1, *argmax_point1])
        loss_log_rand2_dim5.append([i, epoch + 1, loss2, sup_norm2, *argmax_point2])
        loss_log_rand8_dim5.append([i, epoch + 1, loss3, sup_norm3, *argmax_point3])
    ...
    max_diff, argmax_diff = infinity_norm_difference(model_depth2_random, f, random_input_check)
    infinity_norms_rand2_dim5.append([i + 1, max_diff, argmax_diff[0], argmax_diff[1], argmax_diff[2], argmax_diff[3], argmax_diff[4]])

    max_diff, argmax_diff = infinity_norm_difference(model_depth8_random, f, random_input_check)
    infinity_norms_rand8_dim5.append([i + 1, max_diff, argmax_diff[0], argmax_diff[1], argmax_diff[2], argmax_diff[3], argmax_diff[4]])



# Save loss + norm each epoch

with open("Loss random depth 1 width 6 dim 5.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("Loss random depth 2 width 6 dim 5.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Run", "Epoch", "Loss", "Supnorm", "Argmax_x1", "Argmax_x2", "Argmax_x3", "Argmax_x4", "Argmax_x5"])
    writer.writerows(loss_log_rand2_dim5)

with open("Loss random depth 8 width 6 dim 5.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Run", "Epoch", "Loss", "Supnorm", "Argmax_x1", "Argmax_x2", "Argmax_x3", "Argmax_x4", "Argmax_x5"])
    writer.writerows(loss_log_rand8_dim5)

# Final save of parameters
param_df_final = pd.concat(all_params, axis=1)
param_df_final.to_csv("all_params_width6_dim5.csv", index=False)

# Final save of infinity norms and maximizers

infinity_norm_df = pd.DataFrame(infinity_norms_rand1_dim5, columns=["Number", "Max Diff", "Argmax X", "Argmax Y", "Argmax X3", "Argmax X4", "Argmax X5"])
infinity_norm_df.to_csv("NormArgmaxRandom1width6_dim5.csv", index=False)

infinity_norm_df = pd.DataFrame(infinity_norms_rand2_dim5, columns=["Number", "Max Diff", "Argmax X", "Argmax Y", "Argmax X3", "Argmax X4", "Argmax X5"])
infinity_norm_df.to_csv("NormArgmaxRandom2width6_dim5.csv", index=False)

infinity_norm_df = pd.DataFrame(infinity_norms_rand8_dim5, columns=["Number", "Max Diff", "Argmax X", "Argmax Y", "Argmax X3", "Argmax X4", "Argmax X5"])
infinity_norm_df.to_csv("NormArgmaxRandom8width6_dim5.csv", index=False)
